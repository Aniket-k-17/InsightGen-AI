...
if problem_type == "regression":
    ...
    st.info(f"🏷️ **Classification** — model will predict a category ({n_classes} classes)")
...
feature_cols = st.multiselect(
    "📊 Feature columns (you can include text columns too — they are encoded automatically):",
...
# Show which columns are text and will be auto-encoded
text_features = [c for c in feature_cols if df[c].dtype == "object"]
if text_features:
    st.info(f"🔤 Text columns detected: **{', '.join(text_features)}** — these will be auto-converted to numbers for training.")
...
c2.metric("Score",      f"{result['score']}%",
          help="R² for regression (higher=better). Accuracy % for classification.")
c3.metric("Train rows", result["train_rows"])

# BUG FIX: only show MAE for regression (error is None for classification)
...
with tab1:
    if result["problem_type"] == "regression":
        ...
        fig = px.scatter(
            ...
            title="Actual vs Predicted — dots near the green line = good model",
        ...
        # BUG FIX: y_test and y_pred are already decoded strings for classification
        # Comparing them directly works correctly
        actual    = result["y_test"]
        predicted = result["y_pred"]
        ...
        fig = px.bar(
            count_df, x="Result", y="Count",
            title=f"Correct vs Wrong Predictions — {correct}/{len(actual)} correct",
...
with tab2:
    if result["importance"] is not None:
        ...
        st.info("Feature importance is only available for Decision Tree and Random Forest models.")
...
# BUG FIX: round() crashes on strings (classification labels like "Yes"/"No")
# Only round if values are numeric (regression)
if result["problem_type"] == "regression":
    predicted_15 = [round(float(p), 3) for p in predicted_15]
else:
    predicted_15 = [str(p) for p in predicted_15]
    actual_15    = [str(p) for p in actual_15]
...
st.write(f"Fill in values below — the model will predict **{target_col}**:")
...
for i, col in enumerate(feature_cols):
    with input_cols[i % 3]:
        if df[col].dtype == "object":
            # Show a dropdown of all known values from the dataset
            options = sorted(df[col].dropna().unique().tolist())
            user_inputs[col] = st.selectbox(
                f"{col}", options, key=f"inp_{col}"
            )
        else:
...
if st.button("🔮 Predict", use_container_width=True):
    try:
        ...
        # BUG FIX: round() crashes on string classification predictions
        if result["problem_type"] == "regression":
